...
// COMMON_TOOLS is the shared system packages block installed in every image.
const commonTools = `
...
// fuseBuild installs the pre-compiled FUSE binary for kernel-level path transformation.
const fuseBuild = `
...
// pythonToolsBase installs Python dev tools (uv, ruff, mypy, pytest).
const pythonToolsBase = `
# Python 3 runtime
...
// claudeCodeInstall installs Claude Code native binary.
...
// entrypointSetup configures the container entrypoint.
const entrypointSetup = `
WORKDIR /ccbox
...
` + commonTools + fuseBuild + fakepathBuild + claudeCodeInstall + entrypointSetup
}
...
ENV DEBIAN_FRONTEND=noninteractive
` + commonTools + fuseBuild + fakepathBuild + claudeCodeInstall + `
...
var dockerfileGenerators = map[config.LanguageStack]func() string{
	...
	config.StackFunctional: functionalDockerfile,
	config.StackScripting:  scriptingDockerfile,
	config.StackSystems:    systemsDockerfile,

	// Use-Case Stacks
	config.StackData:      dataDockerfile,
	config.StackAI:        aiDockerfile,
	config.StackMobile:    mobileDockerfile,
	config.StackGame:      gameDockerfile,
	config.StackFullstack: fullstackDockerfile,
}

// GenerateDockerfile returns the Dockerfile content for the given stack.
func GenerateDockerfile(stack config.LanguageStack) string {
	gen, ok := dockerfileGenerators[stack]
	if !ok {
